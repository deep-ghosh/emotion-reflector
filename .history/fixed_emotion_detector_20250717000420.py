...
class AdvancedEmotionDetector:
    def __init__(self):
        ...
        # Try to load smile cascade for better smile detection
        try:
    ...
    def detect_emotion_from_face(self, face_gray):
        ...
        # Use smile cascade if available
        if self.smile_cascade is not None:
            smiles = self.smile_cascade.detectMultiScale(face_gray, 1.8, 20)
            if len(smiles) > 0:
                emotion_scores['happy'] += 0.6  # Strong smile detection
                print(f"DEBUG: Smile cascade detected {len(smiles)} smiles")
